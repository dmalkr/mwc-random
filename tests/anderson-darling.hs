...
distributionCDF Normal = \x -> 0.5 * (1.0 + erf (x / sqrt 2.0))
  where
    erf x
        | x < 0 = -erf (-x)
        | otherwise = 
            let a1 =  0.254829592
                ...
                p  =  0.3275911
                t = 1.0 / (1.0 + p * x)
            in 1.0 - (((((a5 * t + a4) * t) + a3) * t + a2) * t + a1) * t * exp (-x * x)
...
main = do
  ...
  xs <- replicateM 1000 $ normal 0 1 g -- 100000 
  -- print xs
